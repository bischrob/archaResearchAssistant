...
class Settings:
    ...
    citation_parser: str = field(default_factory=lambda: _env_str('CITATION_PARSER', 'heuristic').strip().lower())
    reference_parser_llm_model: str = field(default_factory=lambda: _env_str('REFERENCE_PARSER_LLM_MODEL', 'gpt-4.1-mini').strip() or 'gpt-4.1-mini')
    reference_parser_llm_max_references: int = field(default_factory=lambda: _env_int('REFERENCE_PARSER_LLM_MAX_REFERENCES', 12))
    reference_parser_llm_timeout_seconds: int = field(default_factory=lambda: _env_int('REFERENCE_PARSER_LLM_TIMEOUT_SECONDS', 60))
    ...
    query_preprocess_backend: str = field(default_factory=lambda: _env_str('QUERY_PREPROCESS_BACKEND', 'deterministic').strip().lower())
